"""Phase 1 U8: eight untied blocks with mixed top-quartile sequence CE."""
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    """Mix ordinary sequence CE with CE from the hardest batch quartile."""
    ...
    hard_count = max(1, (sequence_losses.numel() + 3) // 4)
    hard_losses = torch.topk(sequence_losses, k=hard_count, sorted=False).values
    return 0.5 * sequence_losses.mean() + 0.5 * hard_losses.mean()
